...
from page_analyzer.validator import normile_url, validatord
from page_analyzer.http_utils import html_parse
...
def create_url():
    ...
    errors = validatord(site)
    if errors:
        for error in errors:
            flash(*error)
        return render_template("index.html",
                               value=site), 422
    url = normile_url(site)
    data = db.get_url(url)
    if data:
        data_id = data.id
...
def check_url(id):
    url = db.get_url(id)
    try:
        url_data = html_parse(url.name)
        url_data["url_id"] = id
        db.insert_url_checks(url_data)
